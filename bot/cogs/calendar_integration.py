...
class CalendarIntegrationCog(commands.Cog):
    ...
    @app_commands.command(name="calendar-export", description="Export tasks to calendar format")
    @app_commands.describe(
        project_id="Project ID to export (optional)",
        format_type="Export format (ical)"
    )
    ...
    @app_commands.command(name="upcoming-deadlines", description="View upcoming task deadlines")
    ...
    async def upcoming_deadlines(
        ...
        for task in upcoming_tasks[:10]:  # Limit to 10 tasks
            ...
            time_text = "Today" if days_until == 0 else f"{days_until} day{'s' if days_until != 1 else ''}"
        ...
        if len(upcoming_tasks) > 10:
            embed.set_footer(text=f"Showing 10 of {len(upcoming_tasks)} upcoming deadlines")
    ...
    async def overdue_tasks(self, interaction: discord.Interaction):
        ...
        user_overdue_tasks = [
            ...
            if any(assignee.discord_id == interaction.user.id for assignee in task.assignees)
        ]
        ...
        if len(user_overdue_tasks) > 10:
            embed.set_footer(text=f"Showing 10 of {len(user_overdue_tasks)} overdue tasks")
